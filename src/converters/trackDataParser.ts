...
export const trackDataFromPoints = (points: TrackPointDto[], name: string): TrackData => {
    const endTime = points[points.length - 1].time;
    const startTime = points[0].time;
    const totalTime = endTime - startTime;

    const velocities: number[] = new Array<number>(points.length);
    velocities[0] = 0;

    const slopes: number[] = new Array<number>(points.length);
    slopes[0] = 0;

    const distances: number[] = new Array<number>(points.length);
    distances[0] = 0;
